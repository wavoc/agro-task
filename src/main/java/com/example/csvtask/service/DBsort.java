...
public class DBsort implements SortService {
    ...
    public List<City> sortedCitiesByCode() {
        return repository.getAllByIdIsNotNullOrderByCityCode().stream().map(CityEntity::getCity).toList();
    }

    @Override
    public List<City> sortedCitiesByName() {
        return repository.getAllByIdIsNotNullOrderByCityName().stream().map(CityEntity::getCity).toList();
    }
}
